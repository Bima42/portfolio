...
export function useHorizontalScroll(options: UseHorizontalScrollOptions): UseHorizontalScrollReturn {
  ...
  const { centerOffset } = useMemo(() => {
    const totalWidth = (options.totalCards - 1) * (options.cardWidth + options.gap);
    const centerOffset = totalWidth / 2;
    
    const segment = 1 / (options.totalCards - 1);
    const indexThresholds = Array.from({ length: options.totalCards }, (_, i) => {
  ...
  const currentIndex = useTransform(scrollYProgress,
    useCallback((progress: number) => {
      ...
      return Math.min(Math.max(0, Math.round(rawIndex)), options.totalCards - 1);
    }, [options.totalCards])
  );
