...
2.  Faire des `grep` dans des flux de texte brut dans un terminal.
3.  Ou pire, demander à un développeur ayant les accès de vérifier pour lui.
...
Nous ne voulions pas d'une usine à gaz comme ELK (Elasticsearch, Logstash, Kibana). Nous avions besoin de quelque chose de léger, qui respecte notre confidentialité (self-hosted) et qui puisse gérer plusieurs projets.
...
Si vous cherchez des tutoriels Loki, 90% d'entre eux vous diront d'utiliser **Promtail**. C'est le collecteur de logs par défaut de la stack.

J'ai choisi **Grafana Alloy** à la place. Voici pourquoi :

1.  **Promtail est en fin de vie.** Grafana consolide activement ses agents. Selon leur [documentation récente](https://grafana.com/docs/loki/latest/send-data/promtail/), Promtail est en mode maintenance.
2.  **Programmabilité.** Alloy utilise une syntaxe proche de Terraform (`.alloy`) qui permet de créer des pipelines puissants. Nous pouvons filtrer, supprimer les PII (Données Personnelles), et re-taguer les logs *avant* qu'ils ne quittent le serveur.
...
J'ai une règle : **Si ça ne marche pas dans `docker-compose`, ça ne part pas en Prod.**

Avant de toucher à notre cloud Jelastic, j'ai construit un Proof of Concept (PoC) en local. L'objectif était de lancer notre stack applicative et de voir les logs apparaître automatiquement dans une instance Grafana locale.
...
Voici la configuration. Notez que nous montons `/var/run/docker.sock` dans Alloy — c'est l'ingrédient secret qui lui permet de découvrir automatiquement les autres conteneurs.
...
services:
  ...
  alloy:
    ...
    ports:
      - "12345:12345" # Expose l'UI d'Alloy pour le débogage
    volumes:
...
```hcl
// 1. DÉCOUVERTE : Trouver tout ce qui tourne sur Docker
discovery.docker "containers" {
  host = "unix:///var/run/docker.sock"
}

// 2. TRAITEMENT : Nettoyer les tags
...
Une fois que nous avons lancé `docker-compose --profile monitoring up`, la différence a été immédiate.

Au lieu d'une fenêtre de terminal noire, nous avons maintenant une base de données d'événements structurée et interrogeable. Je peux aller dans Grafana Explore et lancer une requête comme :

`{project="ophio", env="staging", service="backend"} |= "error"`

![Observation des logs du backend de Ophio par Grafana explore](/src/content/centralizing-logs-with-plg-stack/resources/explore_plg.png)
...
![Dashboard Grafana montrant les logs agrégés des services de Ophio](/src/content/centralizing-logs-with-plg-stack/resources/dashboard_plg.png)
